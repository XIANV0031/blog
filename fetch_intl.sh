#!/usr/bin/env bash
# WAR GAME 情报站 —— 国际周采集脚本
#
# 作用：探测代理 → 采集 Reddit RSS + 境外媒体 → 输出原始素材
# 输出的文件供 AI 读取后撰写「国际周」文章
#
# ⚠️ 本站自 2026-09-20 起改为「一周国内 / 一周国际」双轨轮换：
#   · 国际周（INTL）→ 用本脚本 fetch_intl.sh（境外源，走代理）
#   · 国内周（CN）  → 用 fetch_cn.py（境内源，直连）
#   当前处于哪一周由 week_mode.py 判定。
#
